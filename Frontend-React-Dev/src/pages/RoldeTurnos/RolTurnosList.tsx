...
const ShiftTable: React.FC = () => {
  ...
  // Dato estático por ahora
        ...
        const response = await fetch(
          `${API_BASE_URL}/rol-turnos/Listado?mes=${mesSeleccionado}&anio=2025&especialidadId=${especialidadId}&hospitalId=${establecimiento}`
        );
        if (!response.ok) {
  ...
  const numeroDeDias = new Date(2025, mesSeleccionado, 0).getDate(); // Número de días en el mes seleccionado
  ...
  if (currentDoctorId !== null) {
    ...
    const areasToSave = [
      ...
      fecha: `2025-${mesSeleccionado.toString().padStart(2, '0')}-01`,
    }));
  ...
  <span>
    {
      hospitales.find((h) => h.id === Number(hospitalId))?.nombre ||
      'Hospital no disponible'
          ...
          <tr>
            ...
            {Array.from({ length: numeroDeDias }, (_, i) => {
              const fecha = new Date(2025, mesSeleccionado - 1, i + 1);
              ...
              return (
                <tr key={doctor.doctor_id}>
                  ...
                  {Array.from({ length: numeroDeDias }, (_, i) => {
                    ...
                    const fecha = new Date(2025, mesSeleccionado - 1, parseInt(dia));
  ...
  {!editando && hasPermission(['Admin Hospital']) && (
