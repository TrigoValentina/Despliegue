...
const MiHospital: React.FC = () => {
  ...
  const { establecimientoID, hasPermission } = useAuth();
  ...
  const handleAgregarEspecialidades = () => {
    if (!hasPermission(['Admin Hospital'])) {
      alert('No tienes permisos para agregar especialidades. Solo los administradores del hospital pueden realizar esta acción.');
      return;
    }
  ...
  const handleDeleteClick = (especialidad: Especialidad) => {
    if (!hasPermission(['Admin Hospital'])) {
      alert('No tienes permisos para eliminar especialidades. Solo los administradores del hospital pueden realizar esta acción.');
      return;
    }
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-8 flex justify-end space-x-4">
        ...
        {/* Solo mostrar el botón de agregar si es Admin Hospital */}
          ...
          {especialidades.map((especialidad) => (
            ...
            >
              ...
              <div className="flex space-x-2">
                ...
                {/* Solo mostrar el botón de eliminar si es Admin Hospital */}
                {hasPermission(['Admin Hospital']) && (
                  <button
