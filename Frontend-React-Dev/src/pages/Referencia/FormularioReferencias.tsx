...
const FormularioReferencias: React.FC = () => {
  ...
  try {
    ...
    // Opción 1: Si el endpoint /auth/me devuelve el personal_ID directamente
    // const personalResponse = await axios.get(`${API_BASE_URL}/personal-salud/${authData.personal_ID}`);
    
    // Opción 2: Buscar por usuario_ID (si tu backend lo soporta)
    const personalResponse = await axios.get(`${API_BASE_URL}/personal-salud?usuario_ID=${authData.usuarioID}`);
    const personalData = personalResponse.data.data[0]; // Asumiendo que devuelve un array

    // Opción 3: Si ninguna de las anteriores funciona, podrías necesitar modificar tu backend
    // para que acepte búsquedas por usuario_ID o para que devuelva el personal_ID en /auth/me
  ...
  try {
    const isC12Complete = referencia.fecha_recepcion || referencia.hora_recepcion || referencia.medio_comunicacion;

    const referenciaData = {
      ...referencia,
      establecimiento_salud_referente: referencia.establecimiento_salud_referente || null,
      establecimiento_salud_receptor: isC12Complete ? referencia.establecimiento_salud_receptor || null : null,
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error('Error al crear el registro:', error.response ? error.response.data : error.message);
    } else {
      ...
      <ExpandableSection title="DATOS DEL ESTABLECIMIENTO DE SALUD REFERENTE (C1)">
        <div className="grid-container">
          <div className="form-group">
            ...
            <select value={referencia.establecimiento_salud_referente} disabled style={{ width: '100%' }}>
              <option>
                {
      ...
      <ExpandableSection title="IDENTIFICACIÓN DEL PACIENTE (C2)">
        <div className="grid-container">
          ...
          <div className="form-group">
            ...
            <select value={paciente.tipo_discapacidad} onChange={handlePacienteChange} name='tipo_discapacidad'>
      ...
      <ExpandableSection title="DATOS CLÍNICOS Y SIGNOS VITALES (C3)">
        <div className="grid-container">
          ...
          <div className="form-group">
            ...
            <div className="radio-group">
              <label>
                <input type="radio" name="maduracionPulmonar" value="si" /> Sí
              </label>
              <label>
                <input type="radio" name="maduracionPulmonar" value="no" /> No
              </label>
            </div>
          </div>
        </div>
      ...
      <ExpandableSection title="RESUMEN ANAMNESIS Y EXAMEN FÍSICO (C4)">
        <div className="grid-container">
          <div className="form-group">
            ...
            <div className="radio-group">
              <label>
                <input type="radio" name="estuvoInternado" value="si" /> Sí
              </label>
              <label>
                <input type="radio" name="estuvoInternado" value="no" /> No
              </label>
            </div>
          </div>
          <div className="form-group">
      ...
      <ExpandableSection title="REALIZO EXAMENES COMPLEMENTARIOS DE DIAGNOSTICO (C5)">
        <div className="grid-container">
          <div className="form-group">
            ...
            <div className="radio-group">
              <label>
                <input type="radio" name="realizoExamenes" value="si" /> Sí
              </label>
              <label>
                <input type="radio" name="realizoExamenes" value="no" /> No
              </label>
            </div>
          </div>
          <div className="form-group">
