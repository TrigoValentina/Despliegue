...
async createPersonalSalud_v2(createPersonalSaludDto: CreatePersonalSaludDto): Promise<PersonalSalud> {
  ...
  try {
    ...
    const establecimientoSaludId = currentUser.establecimientoID; // ID del establecimiento del usuario
    let establecimiento: number;

    if (createPersonalSaludDto.establecimiento_salud_idestablecimiento_ID) {
      establecimiento = createPersonalSaludDto.establecimiento_salud_idestablecimiento_ID;
    } else if (establecimientoSaludId) {
      establecimiento = establecimientoSaludId;
    } else {
      throw new NotFoundException('No se pudo determinar el establecimiento. Asegúrate de que el usuario tenga un establecimiento asociado.');
    }

    // Verifica que el establecimiento exista
    ...
    const usuarioData: Partial<Usuario> = {
      ...
      establecimiento_id: establecimiento, // Asegúrate de usar el ID correcto aquí
      fecha_creacion: new Date(),
    };
    ...
    const createPersoEspeciaHospitalDto: CreatePersoEspeciaHospitalDto = {
      ...
      hospital: establecimiento, // usa el establecimiento correcto aquí también
    }; 
