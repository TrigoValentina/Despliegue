...
export class ReferenciaController {
  ...
  async create(@Body() createReferenciaDto: CreateReferenciaDto) {
    ...
    @Post()
async create(@Body() createReferenciaDto: CreateReferenciaDto) {
  const userId = 1; // Ajusta esto según tu lógica
  try {
    return await this.referenciaService.create(createReferenciaDto, userId);
  } catch (error) {
    console.error('Error en el controlador:', error); // Agrega esta línea para depurar
  ...
  async update(
    ...
    const userId = 1; // Usuario de prueba; modificar según sea necesario
