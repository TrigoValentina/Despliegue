...
export class ReferenciaService {
  ...
  try {
    const newReferencia = this.referenciaRepository.create({
      ...createReferenciaDto,
      fecha_recepcion: createReferenciaDto.fecha_recepcion || new Date(),
      hora_recepcion: createReferenciaDto.hora_recepcion || new Date().toTimeString().split(' ')[0],
      usuario_creacion: userId,
      paciente_paciente_ID: { paciente_ID: createReferenciaDto.paciente_paciente_ID } as Paciente,
      establecimiento_salud_receptor: createReferenciaDto.establecimiento_salud_receptor || 1, // Asigna un valor por defecto si no se proporciona
    });

    return await this.referenciaRepository.save(newReferencia);
  } catch (error) {
    console.error('Error al crear referencia:', error);
    throw new InternalServerErrorException('Error al crear la referencia');
  }
}
